...
# legacy stupid code to avoid memory allocations for "zero" tensors
# and unnecessary calculations with "zero" tensors
# e.g. mul two quaternions:
#   (w, x, y, z) * (0, x', y', z')
#   will be replace by:
#   (w, x, y, z) * (None, x', y', z')
#
...
def quaternion_from_two_vectors(v0: torch.Tensor, v1: torch.Tensor) -> torch.Tensor:
    ...
    l0 = (v0 ** 2.0).sum(dim=-2, keepdim=True)  # torch.norm(v0, dim=-2, keepdim=True)
    l1 = (v1 ** 2.0).sum(dim=-2, keepdim=True)  # torch.norm(v0, dim=-2, keepdim=True)
    dot = torch.sum(v0 * v1, dim=-2, keepdim=True)
    w = torch.sqrt(l0 * l1) + dot
...
def rectify_w_of_quaternion(qua: torch.Tensor, inplace=False) -> torch.Tensor:
    ...
    :return: [..., 4, T]
    """
    w_lt = (qua[..., [0], :] < 0.0).expand(qua.shape)  # w less than 0.0
...
def matrix_to_quaternion(matrix):
    ...
    matrix = torch.movedim(matrix, -1, -3)
...
if __name__ == '__main__':
    def foo():
        ...
        print(quaternion_rotate_vector(q, v))
        exit()
        # quaternion (xyzw):
        # [ 0.1276794, 0.1448781, 0.2685358, 0.9437144 ]
        #
        # rotation matrix:
        # [  0.8137977, -0.4698463,  0.3420202;
        #    0.5438381,  0.8231729, -0.1631759;
        #   -0.2048741,  0.3187958,  0.9254166 ]
        #
        eul = torch.tensor([[[10], [20], [30]]])
        qua = euler_to_quaternion(eul, to_rad=3.14159 / 180.0, order="XYZ")
        mtx = quaternion_to_matrix(qua)
        r6d = matrix_to_rotation_6d(mtx)

        print(qua)
        print(mtx)
        print(r6d)

        mtx = rotation_6d_to_matrix(r6d)
        qua = matrix_to_quaternion(mtx)
        print(mtx)
        print(qua)

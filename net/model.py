...
#             }
#         }
#         enc_in_dim = config['enc_in_dim']  # 12
#         enc_nf = config['enc_nf']  # 64
#         latent_dim = config['latent_dim']  # 128
#         graph_cfg = config['graph']  # ...
#
#         self.enc_content = EncoderCon(enc_in_dim, enc_nf, graph_cfg=graph_cfg)
#         self.enc_style = EncoderSty(enc_in_dim, enc_nf, graph_cfg=graph_cfg)
...
#             xabb = xb
#
#         return xaa, xbb, xab, xaba, xabb  # a <- a, b <- b, a <- b(mix), (a <- b) <- a, b <- (a <- b(mix)) OR b
#
#     @staticmethod
...
class EncoderCon(nn.Module):
    def __init__(self, in_channels,
                 channels,
                 graph_cfg,  # 'layout': 'cmu';  'strategy': 'distance', 'max_hop': 2 or 1
                 norm='none',
...
class Decoder(nn.Module):
    ...
    def forward(self, x, *sty):
        ...
        if len(sty) == 1:   # one layer, invoke forward(x, (l1, l2, l3, l4))
            sty = sty[0]    # sty == (G1, G2, G3, BT), sty[::-1] = (BT, G3, G2, G1)
            sty_leftleg = sty_rightleg = sty_spine = sty_leftarm = sty_rightarm = sty[::-1]
        elif len(sty) == 0:
            sty_leftleg = sty_rightleg = sty_spine = sty_leftarm = sty_rightarm = [None, None, None, None]
        elif len(sty) == 4:  # four layers, invoke forward(x, l1, l2, l3, l4)
            sty_leftleg = sty_rightleg = sty_spine = sty_leftarm = sty_rightarm = sty
        else:
            raise Exception("Invalid size of parameter `sty`")

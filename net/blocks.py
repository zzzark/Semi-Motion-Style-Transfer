...
sys.path.append('./net')
from mode_config import USE_ATN, STYLE_CODE_POOL
...
class BPStyleNet(nn.Module):
    def __init__(self,
        ...
        self.gcn1 = SpatialConv(in_channels,
                                in_channels if USE_ATN else out_channels,    # in_channels
                                kernel_size[1],
                                t_kernel_size=1)
        self.tcn1 = nn.Conv2d(in_channels if USE_ATN else out_channels,      # in_channels
                              in_channels if USE_ATN else out_channels,      # in_channels
                              (kernel_size[0], 1),
                              (stride, 1),
                              padding,
        ...
        # rm ----------------- BP-ATN ----------------- #
        if USE_ATN:
            self.astyle_leftleg = ATN(in_channels)
            self.astyle_rightleg = ATN(in_channels)
            self.astyle_spine = ATN(in_channels)
            self.astyle_leftarm = ATN(in_channels)
            self.astyle_rightarm = ATN(in_channels)
    ...
    def forward(self, x,
        ...
        if A.shape[-1] == 21:      # G1 level
            ...
            raise Exception("should be 21 or 10 or 5! Graph is wrong!!")
            # assert A.shape[-1] == 21 or 10 or 5,

        # rm --------- Selective AdaIN --------- #
        if s_leftleg is not None:  # and s_rightleg is not None and ...
            ...
            assert not USE_ATN, "if with ATN, the BP-ATN module can't decode a 'style-less' motion."
...
class SpatialConv(nn.Module):
    ...
    def forward(self, x, A):
        ...
        x = self.conv(x)  # rm First enlarge output size to [N-batch, out * K, T-frame, V-vertex]
        n, kc, t, v = x.size()
        x = x.view(n, self.kernel_size, kc // self.kernel_size, t, v)
        x = torch.einsum('nkctv,kvw->nctw', (x, A))  # rm Then contract to [N-batch, C-out, T-frame, W-vertex] (W=V)
...
class AdaIN(nn.Module):
    def __init__(self, latent_dim, num_features):
        super().__init__()
        pool = nn.AdaptiveAvgPool2d if STYLE_CODE_POOL == 'avg' else nn.AdaptiveMaxPool2d
        self.to_latent = nn.Sequential(pool((1, 1)),
...
class StyleTripletLoss(nn.Module):
    def __init__(self, margin=5.0):
        super(StyleTripletLoss, self).__init__()
        pool = nn.AdaptiveAvgPool2d if STYLE_CODE_POOL == 'avg' else nn.AdaptiveMaxPool2d
        self.pool = pool((1, 1))
        self.loss = nn.ModuleList([
            nn.TripletMarginLoss(margin=margin * 0.2),
            nn.TripletMarginLoss(margin=margin * 0.4),
            nn.TripletMarginLoss(margin=margin * 0.8),
            nn.TripletMarginLoss(margin=margin * 1.0),
        ])

    def forward(self, anc, pos, neg):
        assert isinstance(anc, list) or isinstance(anc, tuple), "The style code should be a list or tuple."
        total = None
        for i, (a, p, n) in enumerate(zip(anc, pos, neg)):
            a = self.pool(a).squeeze(-1).squeeze(-1)
            p = self.pool(p).squeeze(-1).squeeze(-1)
            n = self.pool(n).squeeze(-1).squeeze(-1)
...
class EEVelLoss(nn.Module):
    def __init__(self, index, d='l1'):
        super(EEVelLoss, self).__init__()
        if d == 'l1':
            self.loss = nn.L1Loss()
        elif d == 'l2':
    ...
    def forward(self, inp, ref):
        ...
        (N, V, C, T) / [B, J, 3, T]
        """
        ee_inp = inp[:, self.index, :, :]
        ee_ref = ref[:, self.index, :, :]

        ee_v_inp = ee_inp[..., 1:] - ee_inp[..., :-1]
        ee_v_ref = ee_ref[..., 1:] - ee_ref[..., :-1]

        loss = self.loss(ee_v_inp, ee_v_ref)
        return loss


class EEPosLoss(nn.Module):
    def __init__(self, index, d='l1'):
        super(EEPosLoss, self).__init__()
        if d == 'l1':
            self.loss = nn.L1Loss()
        elif d == 'l2':
            self.loss = nn.MSELoss()
    ...
    def forward(self, inp, ref):
        ...
        (N, V, C, T) / [B, J, 3, T]
        """
        assert inp.shape[-2] == ref.shape[-2] == 3
        ee_inp = inp[:, self.index, :, :]
        ee_ref = ref[:, self.index, :, :]
        loss = self.loss(ee_inp, ee_ref)
        return loss


class EEDistLoss(nn.Module):
    def __init__(self, index, d='l1'):
        super(EEDistLoss, self).__init__()
        if d == 'l1':
            self.loss = nn.L1Loss()
        elif d == 'l2':
            self.loss = nn.MSELoss()
    ...
    def forward(self, inp, ref):
        ...
        ee_inp = inp[:, self.index, :, :]
        ee_ref = ref[:, self.index, :, :]
        ee_d_inp = (ee_inp ** 2.0).sum(dim=2)
        ee_d_ref = (ee_ref ** 2.0).sum(dim=2)
        loss = self.loss(ee_d_inp, ee_d_ref)
        return loss
